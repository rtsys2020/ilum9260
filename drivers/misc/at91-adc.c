...
static int __init adc_init(void)
{
	...
	if(misc_register(&at91_adc_device.miscdev) != 0) {
		ret_val = -EINVAL;
	//		printk("Error is Here ...\n");
		goto error;
	}
	//printk("Creat sysfs files\n");
	...
	//printk("request timer\n");
	/* request timer */
	at91_adc_device.trigger = atmel_tc_alloc(1, AT91_ADC_DEV_NAME);
	if(at91_adc_device.trigger == NULL) {
		ret_val = -EBUSY;
		goto error;
	}
	//printk("default settings\n");
	/* default settings */
	at91_adc_device.settings.res_10bit = 1;
	at91_adc_device.settings.freq = 1000;
	at91_adc_device.settings.ch_mask = 0x0F;
	printk("ADC Driver Loaded!");
	return 0;
